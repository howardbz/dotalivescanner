...
def dist_centroid(cluster_1,cluster_2,norm):
	...
	mean1 = np.mean(cluster_1, axis=0)
	mean2 = np.mean(cluster_2, axis=0)
...
def dist_avg(cluster_1, cluster_2,norm):
	...
	for p1 in cluster_1:
		for p2 in cluster_2:
			templist.append(norm(p1-p2))
	result = np.mean(templist)
	return result
...
def linfnorm(point):
	...
	for feature in point:
		temp = max(temp, abs(feature))
	return temp 

norms_list = [np.linalg.norm, l1norm, linfnorm]
norm_names = ["l2 norm", "l1 norm", "linf norm"]
